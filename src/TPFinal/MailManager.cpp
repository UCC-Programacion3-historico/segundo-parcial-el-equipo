...
void MailManager::addMail(email m) {
    arbolFrom.put(arbolDate.put(m,0),1);
}
 ...
 */
void MailManager::deleteMail(email* del) {
 ...
 */
email** MailManager::getSortedByDate() {
    if(sizeArbol == 0) {
        return tablaOrdenadaDate;
    }
    if(sizeTablaMails == 0) {
        if(sizeArbol == 0) {
            return NULL;
        }
        email** tablaArbol = new email*[sizeArbol];
        for(int i = 0;i<sizeArbol;i++) {
            tablaArbol[i] = arbolNuevosDate->getFirst(arbolNuevosDate);
        }
        tablaOrdenadaDate = tablaArbol;
        return tablaArbol;
    }
    email** tablaDateCompleta = new email*[sizeArbol+sizeTablaMails];

    for(int i = 0;i < sizeTablaMails;i++){
        tablaDateCompleta[i] = tablaOrdenadaDate[i];
    }
    for(int i = sizeTablaMails;i < (sizeTablaMails+sizeArbol);i++) {
        tablaDateCompleta[i] = arbolNuevosDate->getFirst(arbolNuevosDate);
    }
    int j = sizeTablaMails-1;
    while(tablaDateCompleta[sizeTablaMails]->getDateScore() < tablaDateCompleta[j]->getDateScore()){
        j--;
    }
    QuickSortDate(tablaDateCompleta,j,sizeTablaMails+sizeArbol);
    delete tablaOrdenadaDate;
    tablaOrdenadaDate = tablaDateCompleta;
    return tablaDateCompleta;
}
...
void MailManager::initTablaMails() {
    ...
    for(int i=0; i < largo; i++) {
        ...
        try {
            ...
            tablaMails[i].subject = new char[50];
        ...
        if (fgets(tablaMails[i].subject,50,archivo) == NULL) throw 3;
